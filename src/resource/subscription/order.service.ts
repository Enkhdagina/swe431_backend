...
export class OrderService {
    constructor(@InjectModel(Order.name) private model: Model<OrderDocument>, private paymentService: PaymentService, ) {
    ...
    async findUser(user: string) {
        ...
        let res =  await this.model.find({user}).populate('payment').populate('product').exec()
